...
"""producer_v11 — "The Producer" 系フロープランナー + ETA-aware 増援リスク項.

このエージェントは slawekbiel 氏が Kaggle で公開したノートブック
"The Producer V2" の設計をベースにしています:
    https://www.kaggle.com/code/slawekbiel/the-producer-v2
コア（フロー差分スコアラ等）は orbit_lite/ パッケージとして独立に
PyTorch へ移植したもので、本ファイルはその上に V2 の増援リスク項
（reinforce_size_beta 等）を移植・調整した変種です。原設計への敬意と
感謝を込めて、由来を明記します。
"""
...
# orbit_lite はこのファイルの親ディレクトリ（exp1/）に置かれている。
# kaggle_environments が exec() でロードするとき __file__ が未定義になる場合があるため
# 両方のパスを sys.path に追加する。
try:
    _HERE = os.path.dirname(os.path.abspath(__file__))
except NameError:
    _HERE = os.getcwd()
_PARENT = os.path.dirname(_HERE)
for _d in [_HERE, _PARENT]:
...
class ProducerLiteConfig:
    ...
    # --- ETA-aware 増援リスク（V2 移植）-------------------------------------
    # capture_floor を beta * rho(eta) * enemy_mass(target) だけ嵩上げする。
    # flow scorer は敵 do-nothing 前提で投影するため「launch 後に敵が増援できる」
    # ことを見られない。その盲点を補い、飛行中に敵が増援する“負け確の捕獲”に
    # 全駐留兵を突っ込むのを避ける。beta=0 で無効化（v10 と完全等価）。
    reinforce_size_beta: float = 2.2
    reinforce_eta_free: float = 3.0
    reinforce_eta_scale: float = 12.0
    enable_regroup: bool = True
    max_regroup_time: float = 4.0   # v10 維持（V2 既定は 7.0。本変種では増援項のみ分離検証）
...
def plan_lite_waves(
    *,
    ...
    # 到達可能な敵駐留兵の proxy ([P]) を1回だけ計算し、増援リスク floor と
    # regroup gradient の両方で再利用する（[P,P] 縮約を1回で済ませる）。
    ...
    # ETA-aware 増援リスク: capture_floor を beta * rho(k) * enemy_mass(target) だけ
    # 嵩上げ。到着ターン k ごとの増加は rho(k) タイミングランプが担う。beta>0 で発動。
    reinforcement = None
    if beta > 0.0:
    ...
    if not bool(config.enable_regroup):
        return wave_entries
    # 上で計算済みの enemy_mass proxy を再利用（V2 と同じく1回計算で両用途を賄う）。
...
# 4P 共通設定（v5 の CONFIG_4P 相当の保守的近接戦闘 config）。
#
# v9 は pos>=2 に攻撃的な EXPANDER (roi=0.5, def=0, horizon=22 …) を割り当てていたが、
# producer_v9 を直接の対戦相手とした 4P seat-config 探索 (results/v10tune_4p_*) で
# EXPANDER は逆効果と判明した:
#   - fightall (全席 FIGHTER)  : v9 比 +12.9pt (z=+3.08)
#   - expall   (全席 EXPANDER) : v9 比 +0.5pt  (z=+0.11, 同等)
#   - exp3     (3席 EXPANDER)  : v9 比 +0.5pt  (z=+0.11, 同等)
# → EXPANDER を含む構成は v9 と同等、排した構成だけが有意に強い。
# v10 は全 4P 席にこの FIGHTER config を使う（EXPANDER 廃止）。
# horizon=13 と少ない source 数は 4P での actTimeout リスクも下げる。
...
def _config_for(player_count: int, player_id: int = 0) -> ProducerLiteConfig:
    # 2P はデフォルト（v5/v8h/v9 と同一・2P は調整天井）。
    # 4P は全席 FIGHTER（player_id 非依存）。
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict, *, player_id: int):
        mem = self.memory
        if bool((obs_tensors["step"] == 0).all()):
            mem.cached_player_count = None
        if mem.cached_player_count is None:
            mem.cached_player_count = largest_initial_player_count(obs_tensors)
        config = _config_for(mem.cached_player_count, player_id=int(player_id))
        row = run_turn(
...
_DEVICE = "cpu"
# player_id ごとに movement キャッシュを分離（self-play や同一プロセス内 4P 対応）。
# v5 は単一 runtime だったため複数席を同一プロセスで担当するとキャッシュが破壊された。
_RUNTIMES: dict[int, ProducerLiteRuntime] = {}


def _runtime_for(player_id: int) -> ProducerLiteRuntime:
    pid = int(player_id)
    if pid not in _RUNTIMES:
        _RUNTIMES[pid] = ProducerLiteRuntime()
    return _RUNTIMES[pid]
...
def agent(obs):
    ...
    runtime = _runtime_for(player_id)
    if int(step) == 0:
        runtime.reset()
    obs_tensors = single_obs_to_tensor(obs, player_id=player_id, device=_DEVICE)
    with torch.no_grad():
        sparse_row = runtime.tensor_action(obs_tensors, player_id=player_id)
