"""
Orbit Wars - Coordinated Attack Agent (v8h combined)

v8b_quadrant からの派生。v8d の象限ボーナス無効化と v8e の序盤積極化延長を組み合わせる。

差分:
  - _quadrant_strategy_bonus: 常に 1.0（象限・軌道帯によるスコア補正なし、v8d 由来）
  - _is_early_or_small: 50/3 → 80/5（序盤の攻撃しきい値を緩める期間を延長、v8e 由来）
"""
...
def _is_early_or_small(current_step: int, num_my_planets: int) -> bool:
    """v8h: 序盤積極化を80ターン、5惑星まで延長（v8e 由来）。"""
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        # 連携射撃時は手持ち艦（最大 ships_needed まで）を送る
