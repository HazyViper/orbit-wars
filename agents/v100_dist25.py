"""
Orbit Wars - Distance Penalty Tuned Agent (v100)

v60b からの唯一の確認済み改善: DIST_PENALTY = 30.0 → 25.0

グリッドサーチ（12パラメータ × 200試合）の結果:
  DIST_PENALTY=25.0: 55.4% vs v60b（500試合、z=2.41、p≈0.008）
  他11パラメータ変更: 全て統計的に無意味またはノイズ範囲

距離ペナルティを小さくすることで遠距離ターゲットの相対価値が上昇し、
中距離の敵惑星への攻撃積極性が適度に上がる。
"""
...
# ---- チューニング対象パラメータ ----
ATTACK_THRESHOLD      = 1.05   # available >= cost * threshold で攻撃
DIST_PENALTY          = 25.0   # スコア = prod * weight / (1 + dist / DIST_PENALTY)
ENEMY_WEIGHT          = 0.7    # 敵惑星の基本スコア重み
PROXIMITY_BONUS       = 0.5    # 近接ボーナス最大値（dist<PROXIMITY_DIST 時）
PROXIMITY_DIST        = 25.0   # 近接ボーナス適用距離閾値
ROI_NEUTRAL           = 50.0   # 中立惑星 ROI 定数 sqrt(ROI_NEUTRAL / ships_est)
ROI_ENEMY             = 500.0  # 敵惑星 ROI 定数 sqrt(ROI_ENEMY / ships_est)
NEUTRAL_ROI_FILTER    = 20     # step>=150 で ships+1 > prod*N なら中立スキップ
GARRISON_BONUS_FACTOR = 5      # 占領後の初期守備兵 = prod * N
CONCURRENT_DISCOUNT   = 0.5   # 2P 既攻撃ターゲットのスコア割引率
SHIPS_NEEDED_INIT     = 1.1   # 必要艦数の初期推定安全係数
SHIPS_NEEDED_MARGIN   = 1.05  # 必要艦数の収束安全係数
...
def agent(obs):
    ...
    # 惑星数による開拓ボーナスを事前計算（Phase 2.7 と Phase 3 で共有）
    ...
    if num_players <= 2:
        ...
        for mine in my_planets:
            ...
            for t in targets:
                ...
                if base <= 0:
                    continue
                # v60: Phase 2.7 完全スコア一致（orbit_bonus + neutral_bonus + ROI）
        ...
        for t_id, planet_list in frustrated_by_target.items():
            ...
            if t is None:
                continue
            # 各惑星ごとの実距離でTTFを推定（min_distだと遠い惑星の必要艦数を過小評価するバグを修正）
    ...
    # ------------------------------------------------------------------
    # expansion_bonus は Phase 2.7 前に計算済み（再計算不要）
    ...
    # CONCURRENT_DISCOUNT はモジュールレベル定数を使用
    ...
    for mine in my_planets:
        ...
        garrison_bonus = min(int(best_target.production * GARRISON_BONUS_FACTOR), int(available[mine.id]) - ships_needed)
