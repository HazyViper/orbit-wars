"""
Orbit Wars - v105_gar50 (v103b + GARRISON_BONUS_FACTOR=50)

v103b から1パラメータ変更:
  - GARRISON_BONUS_FACTOR: 10 → 50
"""
...
GARRISON_BONUS_FACTOR = 50      # 占領後の初期守備兵 = prod * N
CONCURRENT_DISCOUNT   = 0.5   # 2P 既攻撃ターゲットのスコア割引率
SHIPS_NEEDED_INIT     = 1.15   # 必要艦数の初期推定安全係数
...
def _compute_cumulative_surplus(
    ...
    """正確な最低駐留兵数を計算する（v96: cumulative-surplus 版）。

    turnbyturn 模擬により友軍援軍（空中艦隊）を考慮し、
    garrison 要件を正確に削減する。
    """
...
def agent(obs):
    ...
    #             → 各自軍惑星の最低駐留兵力を計算（cumulative-surplus 版）
    # ------------------------------------------------------------------
