"""
Orbit Wars - Aggressive Improvement Agent (v16)

v8h からの改良版。上位プレイヤー分析に基づき3点を改善:
  1. 攻撃閾値緩和: 1.2x → 1.05x（より積極的に攻撃）
  2. 安価な中立惑星も捕獲（v8hはコスト< 自軍艦数*0.2 を無視していた）
  3. 捕獲時に駐留兵力ボーナス送付（production * 5 追加）
  4. 序盤積極化を 80 → 120 ターンに延長
"""
...
def agent(obs):
    ...
    already_attacking_targets: set[int] = set()  # 2P 連携用
    attacked_targets: set[int] = set()           # 4P 排他用
