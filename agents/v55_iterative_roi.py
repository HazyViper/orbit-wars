"""
Orbit Wars - Expansion Aware Agent (v31)

v29 からの改良版。相手より惑星数が少ない（開拓で負けている）ときに
中立惑星への攻撃スコアをブーストして展開を加速する:
  - expansion_bonus: 相手の最大惑星数 - 自分の惑星数 に比例して中立スコアを増加
  - 最大 1.5x（序盤 step<150）、中盤以降は最大 1.2x に抑制
  - 相手より多い場合は 1.0x（変化なし）
"""
...
def _compute_simple_score(
    ...
    """シンプルな価値関数（v42: v36 ベース + v40 ROI フィルタ＋近接敵ボーナス）:

    距離ペナルティは v36（v31）の /30.0 を維持。
    中立惑星: step>=150 で占拠コスト/生産比が高い惑星をスキップ
    敵惑星:   dist<25 の近接惑星に前哨地除去ボーナス（最大1.5x weight）
    """
    if target.owner == my_player:
        return 0.0
    if target.owner == -1:
        if current_step >= 150 and (target.ships + 1) > target.production * 20:
            return 0.0
        weight = 1.0
    else:
        weight = 0.7
        if distance < 25.0:
            weight *= 1.0 + 0.5 * (1.0 - distance / 25.0)
...
def _estimate_ships_needed(
    ...
    """到着時の予測守備兵を考慮した必要送艦数を推定する。

    中立惑星: 生産しないので現在兵 + 1。
    敵惑星:   送艦数→速度→TTF→守備兵→送艦数 の循環依存を反復で解く。
              振動検出時は保守的に大きい方を採用。(v55: v50 の反復収束版)
    """
    if target.owner == -1 or target.owner == my_player:
        return int(target.ships) + 1

    ships = int(target.ships * 1.1) + 1
    prev = 0
    for _ in range(5):
        speed = fleet_speed(ships)
        ttf = max(1, int(distance / speed))
        garrison = target.ships + target.production * ttf
        new_ships = int(garrison * 1.05) + 1
        if new_ships == ships:
            return ships
        if new_ships == prev:
            # 振動検出: 2値間を往復しているので保守的に大きい方を返す
...
def _should_attack(
    ...
    has_concurrent_attack: 同じターゲットに既に他の自軍惑星から攻撃予定があるか
    """
    ...
    # v16: 閾値を 1.2x → 1.05x に緩和（より積極的な攻撃）
    if mine_ships < cost * 1.05:
        return False
    # v16: 安価な惑星も必ず捕獲（v8h の cost < mine_ships*0.2 制限を撤廃）
    return True
...
def _predict_target_position(
    ...
    if not is_orbiting or angular_velocity == 0.0:
        return t.x, t.y
    ca = math.atan2(t.y - SUN_Y, t.x - SUN_X)  # 現在角度は固定
    dist = math.hypot(mine_x - t.x, mine_y - t.y)
    ttf = int(dist / speed)  # ゲームは整数ステップで動く（v36修正）
    future_x, future_y = t.x, t.y
    for _ in range(6):  # 収束チェック付き最大6回
...
def _compute_threat_multiplayer(
    ...
    """逐次到着モデルによる脅威量計算（v48 修正版）。

    旧版は「最大プレイヤー - 第二プレイヤー」で敵同士の相殺を仮定していたが、
    実際の戦闘は逐次到着（先着艦→守備消費→後着艦→残守備と戦闘）のため、
    弱い敵が先に守備を削ってから強い敵が到達するケースを過小評価していた。

    新版: 全到着艦を時刻順に累積し、各時刻での「累計攻撃 - 生産バッファ」の
    最大値をそのまま脅威量とする。2P では動作が変わらず、4P では保守的だが正確。
    """
    if not incoming:
        return 0.0
...
def _quadrant_strategy_bonus(
    ...
    """v8h: 象限ボーナス完全無効化（v8d 由来）。引数は呼び出し互換のため保持。"""
    return 1.0
...
def _compute_orbit_approach_bonus(
    ...
    """公転標的の接近度に応じた攻撃積極性ボーナス。（2P のみ使用）

    標的が公転中かつ自軍に接近している場合: 最大 1.3（積極的）
    標的が公転中かつ遠ざかっている場合: 最小 0.7（消極的）
    静的標的または angular_velocity == 0: 1.0（変化なし）

    注: 自軍公転惑星の接近は艦隊の直線航路に影響しないため考慮しない。
    """
    orb = math.hypot(t.x - SUN_X, t.y - SUN_Y)
...
def agent(obs):
    ...
    min_garrison_by_planet: dict[int, int] = {}  # v16b: フェーズ 1.5 の援軍判定用
    for mp in my_planets:
        if mp.id in comet_ids:
    ...
    # ------------------------------------------------------------------
    # フェーズ 1.5: 防衛強化（脅威下にある惑星への援軍送信）
    #
    # 自軍惑星の現在兵数が min_garrison を下回る場合（deficit > 0）、
    # 近隣の余裕ある自軍惑星から援軍を送り守備を補強する。
    #
    # タイミング条件:  aid_ttf <= min_enemy_ttf（援軍が敵より先に着く）
    # 優先度:          production × deficit が大きい惑星を先に処理
    # 1惑星1援軍:      最初に条件を満たした helper からのみ送信
    # ------------------------------------------------------------------
    ...
    # high production × large deficit を優先
    ...
    for mp, deficit, min_enemy_ttf in endangered:
        helpers = sorted(
            ...
            key=lambda p: math.hypot(p.x - mp.x, p.y - mp.y),  # 初期ソートは現在位置で十分
        )
        for helper in helpers:
            # 送る量: deficit を埋めるのに足りる量、helper の余剰の半分まで
            send = min(
            ...
            if send < 1:
                continue
            # タイミング確認: 公転惑星の場合は到着時の未来位置でTTFを補正（v49修正）
    ...
    # ------------------------------------------------------------------
    # フェーズ 2.8: 合流攻撃（v26 新規、2P のみ）
    #
    # 単独では落とせないが 2 惑星合計で落とせる高価値敵惑星を検出し、
    # 両惑星が同ターンに直接送艦する。各艦隊が garrison を順次削り最終的に占領。
    # 「自然に同じ標的を最高スコアで選ぶが単独では落とせない惑星ペア」のみ対象
    # → 機会損失ゼロ（どちらの惑星も元からこの標的を選んでいた）
    # 4P は多方面展開が優先のため合流を無効化
    # ------------------------------------------------------------------

    # フェーズ 2.7 / 2.8: 2P のみ（4P は多方面展開優先のため無効化）
    merge_executed: set[int] = set()
    merge_attacked_targets: set[int] = set()

    if num_players <= 2:
        # フェーズ 2.7: 各惑星の自然な最良標的を事前計算（Phase 3 と同スコアリング）
        ...
        for mine in my_planets:
            ...
            for t in targets:
                ...
                if base <= 0:
                    continue
                sc = base * aggression_factor * _compute_target_bonus(t, totals, player)
                if sc > best_sc:
                    best_sc = sc
                    best_t = t
                    best_dist = dist
        ...
        # フェーズ 2.8: 共有「詰まり標的」を検出してジョイント攻撃を実行
        # 同じ標的を自然に選んでいる惑星を標的 ID 別にグループ化
        ...
        for mine in my_planets:
            ...
            # 合流対象: 敵惑星（中立除く）かつ守備兵 20+ のみ。安い標的は Phase 3 に任せる
        ...
        for t_id, planet_list in frustrated_by_target.items():
            ...
            if t is None:
                continue
            # 最も近い攻撃惑星からの距離でTTFを推定
            min_dist = min(math.hypot(p.x - t.x, p.y - t.y) for p in planet_list)
            ships_needed = _estimate_ships_needed(t, min_dist, player)

            # 単独で攻略できる惑星がどこかにある場合はスキップ（Phase 3 が処理）
            if any(available.get(p.id, 0.0) >= ships_needed for p in my_planets
                   if p.id not in comet_ids):
                continue

            # available 降順で上位 2 惑星を選択
            ...
            # 事前バリデーション（移動追加前に両方を確認 → ロールバック不要）
    ...
    # フェーズ 3: 攻撃割当
    #
    # 2P: 連携射撃あり（concurrent 0.5 割引で単独攻撃を優先、割引後も最高なら参加）
    # 4P: v6 と同じ単独攻撃（attacked_targets 排他）
    #
    # 各惑星はスコア最高のターゲット1つを選び、条件を満たせば攻撃。
    # 条件を満たさない場合はその惑星は攻撃しない（v6 と同じ設計）。
    # ------------------------------------------------------------------
    num_my_planets = len(my_planets)

    # 惑星数による開拓ボーナスを計算（v31 新規）
    # 相手の最大惑星数 - 自分の惑星数 = expansion_gap（正 = 自分が負けている）
    enemy_planet_counts: dict[int, int] = defaultdict(int)
    for p in planets:
        if p.owner != -1 and p.owner != player:
            enemy_planet_counts[p.owner] += 1
    max_enemy_planet_count = max(enemy_planet_counts.values()) if enemy_planet_counts else 0
    expansion_gap = max_enemy_planet_count - num_my_planets
    if expansion_gap > 0:
        # 序盤（step<150）は最大 1.5x、以降は最大 1.2x。gap 1 増えるごとに +0.1x
        max_bonus = 1.5 if current_step < 150 else 1.2
        expansion_bonus = min(max_bonus, 1.0 + expansion_gap * 0.1)
    else:
        expansion_bonus = 1.0  # 自分が同数以上なら変化なし

    # 2P: 連携射撃モード, 4P: v6 相当の単独攻撃モード
    allow_concurrent = (num_players <= 2)
    already_attacking_targets: set[int] = merge_attacked_targets  # 合流攻撃済みを引き継ぐ
    attacked_targets: set[int] = merge_attacked_targets.copy()    # 4P 排他用

    CONCURRENT_DISCOUNT = 0.5  # 2P の既攻撃ターゲットスコア割引率
    ...
    for mine in my_planets:
        ...
        for t in targets:
            ...
            # v54: 守備兵コストペナルティ（安い惑星を優先する ROI 補正）
            # ships_needed ∝ garrison, penalty = sqrt(ships_needed / 50) でスムーズに適用
            ships_est = _estimate_ships_needed(t, dist, player)
            if ships_est > 1:
                score *= math.sqrt(50.0 / ships_est)

            # 2P: 既に攻撃中のターゲットは割引して単独攻撃を優先
        ...
        # v16: 基本送艦 + 駐留ボーナス（新惑星に初期守備兵力を残す）
        garrison_bonus = min(int(best_target.production * 5), int(available[mine.id]) - ships_needed)
